package me.firas.skypvp.generators.ironGen;
...
public class IronGenerator {
    ...
    public IronGenerator(WrapperEntityArmorStand entity1, WrapperEntityArmorStand entity2, WrapperEntityArmorStand entity3, WrapperEntityArmorStand entity4){
    ...
    public void tick(){
        if (entityLiving1.isDespawned()){
            IronManager.pool.remove(entityLiving1.getId(), entityLiving1);
            return;
        }
        ...
        for (Entity entity : entityLiving1.getLocation().getWorld().getNearbyEntities(entityLiving1.getLocation(),0,2,0)){
        ...
        if (istrue) {
            if (times <= 0) {
                ...
                entityLiving2.setCustomName(TextHelper.text("&7["+getProgressBar(10-times,10,5,'■', ChatColor.GREEN,ChatColor.RED)+"&7]"));
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        this.cancel();

                        Bukkit.getWorld(entityLiving3.getLocation().getWorld().getUID()).dropItem(entityLiving3.getLocation(), new ItemStack(Material.IRON_INGOT)).setVelocity(new Vector(0, 0.2, 0));
