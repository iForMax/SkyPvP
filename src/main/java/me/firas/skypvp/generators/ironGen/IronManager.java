...
public class IronManager {
    public static ConcurrentHashMap<Integer, IronGenerator> pool = new ConcurrentHashMap<>();
    public static void onStartUp(){
        for (int i = 1; i <= Main.getInstance().getGeneratorsHandler().getValue("Size of Iron"); i++) {
            ...
            entityLiving.setCustomName(TextHelper.text("&8◆ &7&lIron Generator &8◆"));
            ...
            entityLiving4.equip(EnumWrappers.ItemSlot.HEAD,new ItemStack(Material.IRON_BLOCK));
            ...
            IronGenerator hologram = new IronGenerator(entityLiving, entityLiving2, entityLiving3,entityLiving4);
            pool.put(entityLiving.getId(), hologram);
        }
        new BukkitRunnable() {
            @Override
            public void run() {
                for (Map.Entry<Integer, IronGenerator> entry : pool.entrySet()) {
                    try {
                        IronGenerator hologram = entry.getValue();
                        hologram.tick();
