...
public class EmeraldManager {
    public static ConcurrentHashMap<Integer, EmeraldGenerator> pool = new ConcurrentHashMap<>();
    public static void onStartUp(){
        for (int i = 1; i <= Main.getInstance().getGeneratorsHandler().getValue("Size of Emerald"); i++) {
            ...
            entityLiving.setCustomName(TextHelper.text("&8◆ &a&lEmerald Generator &8◆"));
            ...
            entityLiving3.setCustomName(TextHelper.text("&e" + TextHelper.text("60")));
            ...
            entityLiving4.equip(EnumWrappers.ItemSlot.HEAD,new ItemStack(Material.EMERALD_BLOCK));
            ...
            EmeraldGenerator hologram = new EmeraldGenerator(entityLiving, entityLiving2, entityLiving3,entityLiving4);
            pool.put(entityLiving.getId(), hologram);
        }
        new BukkitRunnable() {
            @Override
            public void run() {
                for (Map.Entry<Integer, EmeraldGenerator> entry : pool.entrySet()) {
                    try {
                        EmeraldGenerator hologram = entry.getValue();
                        hologram.tick();
