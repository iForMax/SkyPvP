package me.firas.skypvp.generators.emeraldGen;
...
public class EmeraldGenerator {
    ...
    public EmeraldGenerator(WrapperEntityArmorStand entity1, WrapperEntityArmorStand entity2, WrapperEntityArmorStand entity3, WrapperEntityArmorStand entity4){
    ...
    public void tick(){
        ...
        if (!Main.getInstance().getEvents().isEmeraldEvent()){
        ...
        if (istrue) {
            if (times <= 0) {
                ...
                entityLiving2.setCustomName(TextHelper.text("&7["+getProgressBar(30-times,30,5,'■', ChatColor.GREEN,ChatColor.RED)+"&7]"));
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        this.cancel();
                        Bukkit.getWorld(entityLiving3.getLocation().getWorld().getUID()).dropItem(entityLiving3.getLocation(), new ItemStack(Material.EMERALD)).setVelocity(new Vector(0, 0.2, 0));
